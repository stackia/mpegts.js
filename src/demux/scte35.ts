...
const parseSpliceInsertComponent = (
...
const parseSpliceScheduleEventComponent = (
  reader: ExpGolomb,
): SpliceScheduleEventComponent => {
  const component_tag = reader.readBits(8);
  ...
  if (!spliceInsert.program_splice_flag) {
    ...
    for (let i = 0; i < spliceInsert.component_count; i++) {
      spliceInsert.components.push(
        parseSpliceInsertComponent(spliceInsert.splice_immediate_flag, reader),
      );
    }
  }
...
const parseSegmentationDescriptorComponent = (
  reader: ExpGolomb,
...
const parseAudioDescriptorComponent = (
  reader: ExpGolomb,
): AudioDescriptorComponent => {
  const component_tag = reader.readBits(8);
  const ISO_code = String.fromCharCode(
    reader.readBits(8),
    reader.readBits(8),
    reader.readBits(8),
  );
  ...
  if (splice_command_type === SCTE35CommandType.kSpliceInsert) {
    ...
    if (spliceInsert.splice_event_cancel_indicator) {
      ...
      const auto_return = spliceInsert.duration_flag
        ...
        : undefined;
      const duraiton = spliceInsert.duration_flag
