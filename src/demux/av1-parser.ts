...
class AV1OBUParser {
  static parseOBUs(uint8array: Uint8Array, meta?: AV1Metadata | null) {
    for (let i = 0; i < uint8array.byteLength; ) {
      ...
      if (type === 1) {
        ...
        // OBU_FRAME_HEADER
        meta = AV1OBUParser.parseOBUFrameHeader(
  ...
  static parseSeuqneceHeader(
    uint8array: Uint8Array,
  ): Omit<AV1Metadata, "sequence_header_data"> {
    ...
    if (reduced_still_picture_header) {
      ...
      for (let i = 0; i <= operating_points_cnt_minus_1; i++) {
        ...
        if (decoder_model_info_present_flag) {
          ...
          operating_points[
            operating_points.length - 1
          ].decoder_model_present_for_this_op =
            decoder_model_present_for_this_op;
          if (decoder_model_present_for_this_op) {
            // operating_parameters_info
            let decoder_buffer_delay = gb.readBits(
              buffer_delay_length_minus_1 + 1,
            );
            let encoder_buffer_delay = gb.readBits(
              buffer_delay_length_minus_1 + 1,
            );
  ...
  static parseOBUFrameHeader(
    ...
    if (sequence_header.decoder_model_info_present_flag) {
      ...
      if (buffer_removal_time_present_flag) {
        for (
          let opNum = 0;
          opNum <= sequence_header.operating_points_cnt_minus_1;
          opNum++
        ) {
          if (
            sequence_header.operating_points[opNum]
              .decoder_model_present_for_this_op[opNum]
          ) {
            let opPtIdc =
              sequence_header.operating_points[opNum].operating_point_idc;
            let inTemporalLayer = (opPtIdc >> temporal_id) & 1;
            let inSpatialLayer = (opPtIdc >> (spatial_id + 8)) & 1;
            if (opPtIdc === 0 || (inTemporalLayer && inSpatialLayer)) {
              gb.readBits(
                sequence_header.buffer_removal_time_length_minus_1 + 1,
              );
            }
          }
        }
    ...
    if (
      !(frame_type === SWITCH_FRAME || (frame_type == KEY_FRAME && show_frame))
    ) {
    ...
    if (keyframe) {
      const resolution = AV1OBUParser.frameSizeAndRenderSize(
        gb,
  ...
  static frameSizeAndRenderSize(
    ...
    FrameWidth = Math.floor(
      (UpscaledWidth * 8 /* SUPERRES_NUM */ + SuperresDenom / 2) /
        SuperresDenom,
    );
