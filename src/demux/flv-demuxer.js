...
class FLVDemuxer {
  ...
  _parseAudioData(arrayBuffer, dataOffset, dataSize, tagTimestamp) {
    ...
    if (soundFormat !== 2 && soundFormat !== 3 && soundFormat !== 10) {
      // PCM or MP3 or AAC
      this._onError(
        DemuxErrors.CODEC_UNSUPPORTED,
  ...
  _parseFlacAudioPacket(
    ...
    if (packetType === 0) {
      ...
      this._onError(
        DemuxErrors.FORMAT_ERROR,
        `Flv: Invalid Flac audio packet type ${packetType}`,
      );
      return;
    }
  }
  ...
  _parseFlacSequenceHeader(arrayBuffer, dataOffset, dataSize) {
    ...
    let block_size =
      maximum_block_size === minimum_block_size ? maximum_block_size : null;
    ...
    meta.refSampleDuration =
      block_size != null ? (block_size * 1000) / misc.samplingFrequence : null; // practical encoder sends 4608 blobksize (lower bound limitation)
  ...
  _parseVideoData(
    ...
    if (!isExHeader) {
      ...
      if (fourcc === "hvc1") {
        ...
        this._onError(
          DemuxErrors.CODEC_UNSUPPORTED,
          `Flv: Unsupported codec in video frame: ${fourcc}`,
        );
        return;
      }
    }
  ...
  _parseEnhancedAV1VideoPacket(
    ...
    if (packetType === 0) {
      ...
      this._onError(
        DemuxErrors.FORMAT_ERROR,
        `Flv: Not Supported MP2T AV1 video packet type ${packetType}`,
      );
      return;
    } else if (packetType === 2) {
  ...
  _parseAV1CodecConfigurationRecord(arrayBuffer, dataOffset, dataSize) {
    ...
    const config = AV1OBUParser.parseOBUs(
