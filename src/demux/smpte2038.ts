...
export const smpte2038parse = (data: Uint8Array) => {
  ...
  while (true) {
    let zero = gb.readBits(6);
    readBits += 6;
    if (zero !== 0) {
      break;
    }
    let YC_indicator = gb.readBool();
    readBits += 1;
    let line_number = gb.readBits(11);
    readBits += 11;
    let horizontal_offset = gb.readBits(12);
    readBits += 12;
    let data_ID = gb.readBits(10) & 0xff;
    readBits += 10;
    let data_SDID = gb.readBits(10) & 0xff;
    readBits += 10;
    let data_count = gb.readBits(10) & 0xff;
    readBits += 10;
    let user_data = new Uint8Array(data_count);
    for (let i = 0; i < data_count; i++) {
      let user_data_word = gb.readBits(10) & 0xff;
      readBits += 10;
      user_data[i] = user_data_word;
    }
    let checksum_word = gb.readBits(10);
    readBits += 10;
